        ...
        <table class="kv-grid-table table table-hover  kv-table-wrap">
            <?php foreach ($regions as $region => $region_val){ ?>
                <tr>
                    ...
                    <td><a class="nav-link" <?php echo \app\rbac\WarehouseRule::can('warehouse', 'show-by-type') ? 'href="' . Url::to(['show-by-type']) . '&type=' . $type . '&region=' . $region_val->id . '">' . Yii::t('app', 'View') : '>'; ?> (<?php echo Warehouse::getCountByRegion($type,$region_val->id);?>)</a></td>
                </tr>
            <?php } ?>
        </table>
    </div>
