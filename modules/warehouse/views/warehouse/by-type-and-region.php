        ...
        <table class="table">
            <?php foreach ($communities as $community => $community_val){ ?>
                <tr>
                    ...
                    <td><a class="nav-link" <?php echo \app\rbac\WarehouseRule::can('warehouse', 'show-by-type') ? 'href="' . Url::to(['show-by-type']) . '&type=' . $type . '&community=' . $community_val['id'] . '"' : ''?>><?php echo $community_val['name'];?> (<?php echo Warehouse::getCountByCommunity($type,$community_val['id']);?>)</a></td>
                </tr>
            <?php } ?>
        </table>
    </div>
