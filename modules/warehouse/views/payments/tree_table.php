...
<?php if(array_key_exists('children', $tableTreePartner)){ ?>
    <?php foreach ($tableTreePartner['children'] as $tableTreePartner) : ?>
        <li class="file-tree-folder" style="display: block;"> <span data-name="<?= $tableTreePartner['name'] ?>" class="parent-block"><?= $tableTreePartner['name'] ?>
            <?php if(!array_key_exists('children', $tableTreePartner)){ ?>
               <?php echo ' <small style="margin-left:20px;">Պարտք`'.number_format(ShippingRequest::getPartnerTotalAmount($tableTreePartner['id']),0,'.',',').' Դրամ  <span style="cursor:pointer;" onclick="showInvoices('.$tableTreePartner['id'].')" data-toggle="modal" data-target="#viewInfo">(invoices)</span><samll>'; ?>
            <?php } ?>
            </span>
