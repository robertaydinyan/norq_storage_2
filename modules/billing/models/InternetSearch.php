...
/**
 * InternetSearch represents the model behind the search form of `app\modules\billing\models\Internet`.
 */
class InternetSearch extends Internet
{
    ...
    {
        return [
            [['id', 'speed', 'inet_speed_unit_id', 'size', 'inet_size_unit_id'], 'integer'],
            [['price'], 'number'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere([
            'id' => $this->id,
            'speed' => $this->speed,
            'inet_speed_unit_id' => $this->inet_speed_unit_id,
            'size' => $this->size,
            'inet_size_unit_id' => $this->inet_size_unit_id,
            'price' => $this->price,
        ]);
    ...
    public function search_new( $page = 1,  $sort = 'none' , $column = '' , $dataSearch=''){
        $query = Internet::find();
        $columns = TableSettings::find()->where(['user_id' => Yii::$app->user->id, 'page'=>'/internet'])->asArray()->one();
        ...
        if(!empty($filter_all)){
            ...
            $total = intval(Internet::find()->count());
        }
    ...
    public function updateSortData($column, $sort){
        $model = TableSettings::find()->where(['user_id'=>Yii::$app->user->id, 'page'=>'/internet'])->one();
        if(!empty($model)){
    ...
    public function makeTable($table, $columns, $page, $count_show, $total, $column, $sort, $pined) {
        ...
        $columns_all_search = [];
        $l = 0;
        $searchMemory = SearchSettings::find()->where(['user_id' => Yii::$app->user->id, 'page' => '/internet'])->all();
        $list = [];
        $arrTable['items_all'] = [];
            ...
            foreach ($searchMemory as $search => $vals) {
                ...
                for ($k = 0; $k < count($col_search); $k++) {
                    ...
                    if (isset($column_search[1])) {
                        ...
                        $arrTable['search'][$l]['items'][] = ['alias' => $column_search[0], 'name' => Internet::getAttributeLabel($column_search[0]), 'is_visible' => $stat, 'value' => intval($new_res[1])];
                    }
                }
                $l++;
            }
                ...
                foreach ($keys as $ks => $v) {
                    ...
                    if($column == '') {
                        $arrTable['header'][$ks] = [
                            'align' => 'left',
                            'checked' => true,
                            'name' => Internet::getAttributeLabel($v),
                            'is_visible' => $visible,
                            'list' => $list,
                            'list_all' => $list,
                            'class' => 'new',
