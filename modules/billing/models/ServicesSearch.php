...
/**
 * ServicesSearch represents the model behind the search form of `app\modules\billing\models\Services`.
 */
class ServicesSearch extends Services
{
    ...
    {
        return [
            [['id', 'services_client_type', 'payment_type', 'payment_period', 'random', 'is_all_country'], 'integer'],
            [['name'], 'safe'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere([
            'id' => $this->id,
            'services_client_type' => $this->services_client_type,
            'payment_type' => $this->payment_type,
            'payment_period' => $this->payment_period,
            'random' => $this->random,
            'is_all_country' => $this->is_all_country,
        ]);
    ...
    public function search_new($page = 1,  $sort = 'none', $column = '', $dataSearch = ''){
        $query = Services::find()->select(['id', 'name', 'services_client_type', 'payment_period', 'random']);
        $columns = TableSettings::find()->where(['user_id' => Yii::$app->user->id, 'page'=>'/services'])->asArray()->one();
        ...
        if(!empty($filter_all)){
            ...
            $total = intval(Services::find()->count());
        }
        ...
        foreach($table as $k => $t) {
            ...
            $arrTable[$k]['id'] = $t->id;
            $arrTable[$k]['name'] = $t->name;
            $arrTable[$k]['services_client_type'] = $t->services_client_type == 0 ? Yii::t('app', 'Корпоративный') : Yii::t('app', 'Для физ.лиц');
            $arrTable[$k]['payment_period'] = $t->payment_period == 0 ? Yii::t('app', 'Помесячная') : Yii::t('app', 'Произвольная');
            $arrTable[$k]['random'] = $t->random;
        }
    ...
    public function makeTable($table, $columns, $page, $count_show, $total, $column, $sort, $pined) {
        ...
        $columns_all_search = [];
        $l = 0;
        $searchMemory = SearchSettings::find()->where(['user_id' => Yii::$app->user->id, 'page' => '/tarservicesiff'])->all();
        $list = [];
            ...
            foreach ($searchMemory as $search => $vals) {
                ...
                for ($k = 0; $k < count($col_search); $k++) {
                    ...
                    if (isset($column_search[1])) {
                        ...
                        $arrTable['search'][$l]['items'][] = ['alias' => $column_search[0], 'name' => Services::getAttributeLabel($column_search[0]), 'is_visible' => $stat, 'value' => intval($new_res[1])];
                    }
                }
                $l++;
            }
                ...
                foreach ($keys as $ks => $v) {
                    ...
                    if($column == '') {
                        $arrTable['header'][$ks] = [
                            'align' => 'left',
                            'checked' => true,
                            'name' => Services::getAttributeLabel($v),
                            'is_visible' => $visible,
                            'list' => $list,
                            'list_all' => $list,
                            'class' => 'new',
