...
/**
 * TvChannelSearch represents the model behind the search form of `app\modules\billing\models\TvChannel`.
 */
class TvChannelSearch extends TvChannel
{
    /**
     * {@inheritdoc}
     */
    ...
    public function search_new( $page = 1,  $sort = 'none' , $column = '' , $dataSearch=''){
        $query = TvChannel::find()->select('id,name,cost_price,amount,active');

        $columns = TableSettings::find()->where(['user_id' => Yii::$app->user->id, 'page'=>'/tv-channel'])->asArray()->one();
        ...
        if(!empty($filter_all)){
            $total = TvChannel::find()->select('id,name,cost_price,amount,active');
            ...
            $total = intval(TvChannel::find()->count());
        }
    ...
    public function updateSortData($column, $sort){
        $model = TableSettings::find()->where(['user_id'=>Yii::$app->user->id, 'page'=>'/tv-channel'])->one();
        if(!empty($model)){
    ...
    public function makeTable($table, $columns, $page, $count_show, $total, $column, $sort, $pined) {
        ...
        $columns_all_search = [];
        $l = 0;
        $searchMemory = SearchSettings::find()->where(['user_id' => Yii::$app->user->id, 'page' => '/tv-channel'])->all();
        $list = [];
        $arrTable['items_all'] = [];
            ...
            foreach ($searchMemory as $search => $vals) {
                ...
                for ($k = 0; $k < count($col_search); $k++) {
                    ...
                    if (isset($column_search[1])) {
                        ...
                        $arrTable['search'][$l]['items'][] = ['alias' => $column_search[0], 'name' => TvChannel::getAttributeLabel($column_search[0]), 'is_visible' => $stat, 'value' => intval($new_res[1])];
                    }
                }
                $l++;
            }
                ...
                foreach ($keys as $ks => $v) {
                    ...
                    if($column == '') {
                        $arrTable['header'][$ks] = [
                            'align' => 'left',
                            'checked' => true,
                            'name' => TvChannel::getAttributeLabel($v),
                            'is_visible' => $visible,
                            'list' => $list,
                            'list_all' => $list,
                            'class' => 'new',
