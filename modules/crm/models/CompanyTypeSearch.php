...
/**
 * CompanyTypeSearch represents the model behind the search form of `app\modules\crm\models\CompanyType`.
 */
class CompanyTypeSearch extends CompanyType
{
    ...
    public function search_new( $page = 1,  $sort = 'none' , $column = '' , $dataSearch=''){
        ...
        $columns = TableSettings::find()->where(['user_id' => Yii::$app->user->id, 'page'=>'/company-type'])->asArray()->one();
        ...
        if(!empty($filter_all)){
            ...
            $total = intval(CompanyType::find()->count());
        }
    ...
    public function updateSortData($column, $sort){
        $model = TableSettings::find()->where(['user_id'=>Yii::$app->user->id, 'page'=>'/company-type'])->one();
        if(!empty($model)){
    ...
    public function makeTable($table, $columns, $page, $count_show, $total, $column, $sort, $pined) {
        ...
        $columns_all_search = [];
        $l = 0;
        $searchMemory = SearchSettings::find()->where(['user_id' => Yii::$app->user->id, 'page' => '/company-type'])->all();
        $list = [];
        $arrTable['items_all'] = [];
            ...
            foreach ($searchMemory as $search => $vals) {
                ...
                for ($k = 0; $k < count($col_search); $k++) {
                    ...
                    if (isset($column_search[1])) {
                        ...
                        $arrTable['search'][$l]['items'][] = ['alias' => $column_search[0], 'name' => CompanyType::getAttributeLabel($column_search[0]), 'is_visible' => $stat, 'value' => intval($new_res[1])];
                    }
                }
                $l++;
            }
                ...
                foreach ($keys as $ks => $v) {
                    ...
                    if($column == '') {
                        $arrTable['header'][$ks] = [
                            'align' => 'left',
                            'checked' => true,
                            'name' => CompanyType::getAttributeLabel($v),
                            'is_visible' => $visible,
                            'list' => $list,
                            'list_all' => $list,
                            'class' => 'new',
...
//                        varDumper((object) $table);
            ...
            $arrTable['menu'][0] = [
                ...
                'url' => Url::to(['/crm/company-type/view']),
                'form' => ''
            ];
            ...
            $arrTable['menu'][1] = [
                ...
                'url' => Url::to(['/crm/company-type/update'   ]),
                'form' => 'Редактировать'
            ];
            ...
            $arrTable['menu'][2] = [
                ...
                'method' => 'post',
                'class' => 'open_docs_view',
                'url' => Url::to(['/crm/company-type/delete' ]),
                'form' => ''
            ];
