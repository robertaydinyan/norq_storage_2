...
class TimeHelper
{
    ...
    public static function getTimeAgoInWord($timestamp)
    {
        ...
        // Few seconds ago
        if ($difference < 15) {
            return \Yii::t('app', 'Few seconds ago');
        } // Seconds ago
        else if ($difference < 60) {
            return \Yii::t('app', '{0, plural, =1{one second ago} other{# seconds ago}}', $difference);
        } // Minutes ago
        else if ($difference < 60 * 60) {
            $minutes = round($difference / 60);
            return \Yii::t('app', '{0, plural, =1{1 minute ago} other{# minutes ago}}', $minutes);
        } // Hours ago
        else if ($difference < 24 * 60 * 60) {
            $hours = round($difference / 60 / 60);
            return \Yii::t('app', '{0, plural, =1{1 hour ago} other{# hours ago}}', $hours);
        } // Days ago
        else if ($difference < 7 * 24 * 60 * 60) {
            $days = round($difference / 24 / 60 / 60);
            return \Yii::t('app', '{0, plural, =1{1 day ago} other{# days ago}}', $days);
        } // Weeks ago
        else if ($timestamp > strtotime('-1 month')) {
            $weeks = round($difference / 7 / 24 / 60 / 60);
            return \Yii::t('app', '{0, plural, =1{1 week ago} other{# weeks ago}}', $weeks);
        } // Months ago
        else if ($timestamp > strtotime('-1 year')) {
            $interval = date_diff((new \DateTime(static::getTime($timestamp))), (new \DateTime()));
            return \Yii::t('app', '{0, plural, =1{1 month ago} other{# months ago}}', $interval->m);
        }

        // Years ago
        $interval = date_diff((new \DateTime(static::getTime($timestamp))), (new \DateTime()));
        return \Yii::t('app', '{0, plural, =1{1 year ago} other{# years ago}}', $interval->y);
    }
     ...
     * @return float|int
     */
    public static function getMonthsBetweenDates($startDate, $endDate) {
        $to = Carbon::createFromFormat('Y-m-d H:s:i', $startDate);
     ...
     */
    public static function localizedDate($date, $locale = 'hy')
    {
     ...
     * @return array
     */
    public static function getMonthListFromDate()
    {
        $start = date('Y-01-01');
        $end = date('Y-12-31');
        foreach (CarbonPeriod::create($start, '1 month', $end) as $key => $month) {
            $months[$key + 1] = StringHelper::mb_ucfirst($month->translatedFormat('F'));
        }
