...
class UserSearch extends User
{
    ...
    public function search_new($page = 1, $sort = 'none', $column = '', $dataSearch = '') {
        ...
        foreach ($table as $key => $tableItem) {
            ...
            $arrTable[$key]['status'] = $tableItem['status'] == User::STATUS_ACTIVE ? Yii::t('app', 'Активный') : Yii::t('app', 'Неактивный');
            $arrTable[$key]['role'] = array_key_exists($tableItem['role'], User::getRole()) ? User::getRole()[$tableItem['role']] : Yii::t('app', 'Неизвестная роль');
    ...
    public function makeTable($table, $columns, $page, $count_show, $total, $column, $sort, $pined) {
        ...
        if(!empty($table)) {
            ...
            $arrTable['search'][$l]['name'] = Yii::t('app', 'Поиск');
